...
# Check if anonbird-ui is running
pid="$(pgrep -x -f /usr/bin/anonbird-ui || true)"
if [ -n "${pid}" ]
then
  ...
  pkill -x -f /usr/bin/anonbird-ui >/dev/null 2>&1
  su - "${username}" -c 'nohup /usr/bin/anonbird-ui > /dev/null 2>&1 &'
fi
