...
Закрыто: итоговый release report/verdict и последняя known-limitations сверка перед stable/open-source announcement выполнены в `docs/release-report-v0.72.2.md`. Clean install server/dashboard из опубликованных images, RHEL-like client install, published latest update smoke, server-side NetBird baseline migration + rollback, двухклиентный client migration + rollback/reapply и Marton Tor/I2P overlay checks закрыты отдельными E2E/status entries.
