...
- Текущий фокус: Phase 1-4, Phase 5 Tor relay stream multipath/per-channel health и Phase 6 direct-I2P MVP закрыты на code/unit + стендовом уровне; leak-map audit закрывает найденные clearnet side channels, reconnect/failure soak прошёл, dashboard/runtime/release packaging/proxy web/docs/infrastructure surface hardened. Production-readiness слой почти закрыт: published RC7 artifacts/images, stable `v0.72.1` artifacts/images, full public-main CI, Marton Tor repeat из release artifact, NetBird->AnonBird migration server+clients, rollback/uninstall/reinstall, one-command self-host и update smoke пройдены; остаются hardened stable release rerun после Windows Mesa3D dependency fix и итоговый open-source release report/verdict.
- Оценка остатка на 2026-06-01 после open-source hygiene, GitHub/local rename, public repo switch, зелёного RC2/RC4/RC6/RC7 tag-release workflow, public GHCR manifests, clean one-command self-host из опубликованных образов, зелёного public-main CI, двухклиентной NetBird->AnonBird client migration с rollback/reapply, published RC7 update smoke, stable `v0.72.1` release artifact и Marton Tor overlay repeat из RC7: примерно 99.2% от ТЗ закрыто; главный незакрытый блок — получить полностью зелёный stable tag-release после hardening Windows Mesa3D source, затем финальный release report/verdict и последняя сверка known limitations перед open-source announcement.
...
Открыто: итоговый release report/verdict и последняя known-limitations сверка перед stable/open-source announcement. Clean install server/dashboard из опубликованных images, RHEL-like client install, published RC7 update smoke, server-side NetBird baseline migration + rollback, двухклиентный client migration + rollback/reapply и Marton Tor/I2P overlay checks закрыты отдельными E2E/status entries.
