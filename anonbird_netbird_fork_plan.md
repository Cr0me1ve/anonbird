...
Открыто: clean install server/dashboard artifacts из опубликованных images, RHEL-like client install, uninstall/reinstall, двухклиентный NetBird baseline migration, Marton service test. Server-side NetBird baseline migration + rollback и single live-root client migration + rollback на RC-local/dev artifact закрыты отдельными E2E.
