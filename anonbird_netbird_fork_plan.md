...
Открыто: двухклиентный NetBird baseline migration, Marton Tor-repeat/release-artifact repeat и итоговый release report. Clean install server/dashboard из опубликованных images, RHEL-like client install, server-side NetBird baseline migration + rollback и single live-root client migration + rollback закрыты отдельными E2E/status entries.
