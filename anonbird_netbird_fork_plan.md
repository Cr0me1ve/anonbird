...
- Оценка остатка на 2026-06-01 после open-source hygiene, GitHub/local rename, public repo switch, зелёного RC2/RC4/RC6 tag-release workflow, public GHCR manifests, clean one-command self-host из опубликованных образов, зелёного public-main CI, двухклиентной NetBird->AnonBird client migration с rollback/reapply и Marton Tor overlay repeat: примерно 98.7% от ТЗ закрыто; главный незакрытый блок — новый опубликованный RC/release artifact с последними installer/migration update fixes, repeat ключевых smoke из этого artifact и финальный release report/verdict.
...
Открыто: повторить минимальный Marton smoke из следующего release artifact/RC that includes latest migration hardening, because текущий Tor repeat used the migrated testbed with one local fixed client binary during the migration hardening validation.
...
Открыто: новый RC/release artifact с latest migration hardening, короткий repeat install/migration/Marton smoke из этого artifact и итоговый release report. Clean install server/dashboard из опубликованных images, RHEL-like client install, server-side NetBird baseline migration + rollback, двухклиентный client migration + rollback/reapply и Marton Tor/I2P overlay checks закрыты отдельными E2E/status entries.
