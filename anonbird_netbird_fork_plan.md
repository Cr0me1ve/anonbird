...
- Статус: MVP anonymous mesh реализован и прошёл requirement-by-requirement audit 2026-05-31; post-MVP production/open-source release gate закрыт на stable `v0.72.2` 2026-06-01.
- Текущий фокус: Phase 1-4, Phase 5 Tor relay stream multipath/per-channel health и Phase 6 direct-I2P MVP закрыты на code/unit + стендовом уровне; leak-map audit закрывает найденные clearnet side channels, reconnect/failure soak прошёл, dashboard/runtime/release packaging/proxy web/docs/infrastructure surface hardened. Production-readiness слой закрыт: stable `v0.72.2` artifacts/images, full public-main CI, full stable tag release CI including Windows installer `amd64`, one-command self-host из `releases/latest`, NetBird->AnonBird migration server+clients, rollback/uninstall/reinstall, Marton Tor repeat из release artifact, published latest update smoke и final release report/verdict пройдены.
- Оценка на 2026-06-01 после open-source hygiene, GitHub/local rename, public repo switch, зелёного RC2/RC4/RC6/RC7 tag-release workflow, public GHCR manifests, clean one-command self-host из опубликованных образов, зелёного public-main CI, двухклиентной NetBird->AnonBird client migration с rollback/reapply, stable `v0.72.2` tag-release, latest self-host smoke, latest client update smoke и release reports `docs/release-report-v0.72.2.md`/`docs/release-report-v0.72.2-ru.md`: 100% текущего production/open-source release gate по ТЗ закрыто. Дальнейшие работы — улучшения после релиза, code signing/notarization и расширение матрицы, а не blocker для текущего verdict.
...
- Заметка: для ручной Windows build/test проверки доступен ПК пользователя по SSH `gay--@192.168.0.100`; использовать как fallback runner/verification host, когда GitHub-hosted Windows CI или external dependencies дают сомнительный результат.
...
Дополнительный build/test resource 2026-06-01: Windows PC доступен по SSH `gay--@192.168.0.100`; использовать для Windows сборок/ручной проверки installer при необходимости. Не хранить credentials в git/artifacts.
...
Статус 2026-06-01: one-command self-host default дополнительно приведён к anonymous-by-default policy: generated `config.yaml` теперь пишет `stunPorts: []`, Docker Compose не публикует `3478/udp`, а clearnet/STUN legacy режим доступен только через явный `--enable-clearnet-stun` или `ANONBIRD_ENABLE_CLEARNET_STUN=true`. README quickstart дополнен operator checks (`docker compose ps`, OIDC discovery, `/api/users` -> `401`), setup-key bootstrap командой и предупреждением, что STUN/UDP включается только при осознанном real-IP exposure risk. Проверено локально: `bash -n infrastructure_files/getting-started.sh`, `shellcheck -S error infrastructure_files/getting-started.sh`, `--render-only` default + `docker compose config` (только `stunPorts: []`, без `3478`), `--render-only --enable-clearnet-stun` + `docker compose config` (`stunPorts: [3478]` и published UDP `3478`), `git diff --check`.

Статус 2026-06-01: tag-release workflow hardened для open-source окружения без приватного signing pipeline: `trigger_signer` теперь запускается только при tag push и `vars.ANONBIRD_SIGN_PIPELINES_ENABLED == 'true'`; иначе release artifacts/images могут быть опубликованы без падения на отсутствующем `SIGN_GITHUB_TOKEN`/private repo. Проверено: `actionlint .github/workflows/release.yml`, `git diff --check`.

Статус 2026-06-01: RC tag `v0.72.0-rc.1` был создан как первый настоящий publishing gate и выявил tag-only blocker: `release_ui` и `release_ui_darwin` в release mode пытаются создать/update GitHub Release, но jobs не имели `contents: write`, поэтому GoReleaser падал с `403 Resource not accessible by integration`; snapshot runs этого не ловили. Исправлено: для `release_ui` и `release_ui_darwin` добавлены `permissions: contents: write`. Открыто: повторить tag gate на следующем RC tag.

Статус 2026-05-31: published-image gate preflight показал, что `ghcr.io/cr0me1ve/anonbird-dashboard:latest`, `ghcr.io/cr0me1ve/anonbird-server:latest` и `ghcr.io/cr0me1ve/anonbird-reverse-proxy:latest` пока недоступны для manifest inspect (`manifest unknown`; без auth earlier response was `denied`). Для server/proxy release path исправлен GitHub Actions blocker: `release` job теперь явно запрашивает `contents: write` и `packages: write`, а GHCR login использует `secrets.CI_DOCKER_PUSH_GITHUB_TOKEN || github.token`, чтобы tag release мог публиковать GHCR images без отдельного PAT. Дополнительно release workflow очищен под open-source lint: standard `ubuntu-latest` runner label, shell-safe FreeBSD diff detection, quoted `$GITHUB_ENV`/`$GITHUB_PATH`, artifacts JSON через env вместо embedded expression. Follow-up hardening: `.goreleaser.yaml` теперь GHCR-only для container images/manifests, Docker Hub/Homebrew publishing больше не required для fork release path, RPM signing/verification выполняются только при наличии `GPG_RPM_PRIVATE_KEY`, поэтому пустой secrets set не должен ломать tag release. GoReleaser deprecation cleanup закрыт: `archives.builds`/`nfpms.builds` заменены на `ids`, `archives.format` на `formats`, legacy `dockers`/`docker_manifests` заменены на `dockers_v2` multi-platform GHCR images, Dockerfile copy paths адаптированы к `TARGETPLATFORM` build context. Проверено: YAML parse OK, `actionlint .github/workflows/release.yml` OK, `goreleaser check --config .goreleaser.yaml`, `goreleaser check --config .goreleaser_ui.yaml`, `goreleaser check --config .goreleaser_ui_darwin.yaml` проходят без deprecation-fail, `git diff --check` OK. Локальный Docker daemon не запущен, поэтому фактический image build/push остаётся частью tag/release gate. Открыто: сделать tag/release run, подтвердить public GHCR manifests и повторить one-command self-host без image overrides.

Статус 2026-05-31: public `main` snapshot release run `26722677963` после исправления runner label дошёл до GoReleaser image build, но упал на Docker `linux/arm/v6`: upstream `gcr.io/distroless/base:debug` не содержит armv6 manifest (`no match for platform in manifest`). Исправлено: Docker publish matrix в `.goreleaser.yaml` ограничена `linux/amd64` + `linux/arm64`; armv6 бинарники/архивы при этом остаются в release build matrix. Проверено: `goreleaser check --config .goreleaser.yaml`, `actionlint .github/workflows/release.yml`. Открыто: повторить release workflow после push/tag, затем подтвердить GHCR manifests.
...
- [x] Переименовать локальные рабочие папки на компьютере:
...
Статус 2026-05-31: GitHub repositories переименованы: `Cr0me1ve/netbird` -> `Cr0me1ve/anonbird`, `Cr0me1ve/dashboard` -> `Cr0me1ve/anonbird-dashboard`. Локальные git remotes обновлены на новые SSH URLs. Локальные папки переименованы: `/Users/kirill/Code/netbird` -> `/Users/kirill/Code/anonbird`, `/Users/kirill/Code/dashboard` -> `/Users/kirill/Code/anonbird-dashboard`. Основной repo и dashboard больше не содержат ссылок `Cr0me1ve/netbird`/`Cr0me1ve/dashboard` в tracked source/docs outside `.git`; они заменены на `Cr0me1ve/anonbird`/`Cr0me1ve/anonbird-dashboard`. Go module path остаётся compatibility decision и пока намеренно не переписан.
...
- [x] Нарисовать новый минималистичный логотип AnonBird:
  ...
  - без рогов и без трезубца по последнему уточнению;
...
- [x] Заменить старые logo/icon assets в `anonbird` и `anonbird-dashboard`, пересобрать UI/proxy/dashboard bundles и проверить light/dark backgrounds.
...
Статус 2026-05-31: package/install локальный preflight после AnonBird naming pass закрыт (`sh -n`, `shellcheck -S error`, YAML parse, WiX XML parse, targeted updater/cmd Go tests). Дополнительно закрыт self-host quickstart preflight и RC stack smoke: `getting-started.sh --render-only` генерирует валидный Compose stack, `--preflight-only`/image override path даёт fail-fast проверку release images до запуска контейнеров, а `93.177.116.58` подтвердил реальный dashboard/management startup через Traefik с RC-local images и без старых upstream release/geolocation fetches после fix. В `Code/anonbird-dashboard` исправлен GHCR publishing workflow: PR builds no longer push, release tags publish semver/ref tags and `latest`, package metadata/docs match `anonbird-dashboard`; verified with `actionlint`, YAML parse and `npm run build`. В `anonbird` release workflow добавлены explicit `contents: write`/`packages: write` permissions и GHCR login fallback на `github.token`; затем workflow очищен до `actionlint` OK и hardened как GHCR-only без обязательных Docker Hub/Homebrew secrets. GoReleaser config теперь проходит clean check на версии CI (`v2.14.3`) без deprecated release fields; container publish path переведён на `dockers_v2` multi-platform GHCR images. Open-source secret hygiene добавлен и проверен: `gitleaks dir` + `gitleaks git` clean для основного repo (6245 commits) и dashboard (699 commits), repo secrets/variables в GitHub пустые. Repos переведены в public: `Cr0me1ve/anonbird`, `Cr0me1ve/anonbird-dashboard`; `main` fast-forwarded до текущей AnonBird реализации. Первый public `main` push выявил два CI blockers: transient account billing/spending-limit failure on some jobs and unavailable custom runner label `ubuntu-latest-8-cores` for main release job. `release_ui` and `release_ui_darwin` snapshot jobs passed; main release job queued on `ubuntu-latest-8-cores`, so release workflow was fixed to use standard `ubuntu-latest`. Текущий GHCR manifest preflight для default quickstart images пока возвращает `manifest unknown`, поэтому published-image clean install остаётся открытым до tag/release run и public pull smoke. Server-side migration E2E + rollback из upstream NetBird `v0.64.6` теперь проверены на `93.177.116.58`. Это не заменяет полный release suite: remote artifact install из опубликованных server/dashboard images, dashboard/proxy build, client migration E2E и Marton overlay test остаются обязательными.

Статус 2026-05-31: следующий public `main` snapshot release run `26722677963` подтвердил, что runner issue снят и `release_ui`/`release_ui_darwin` проходят, но main GoReleaser упал на Docker `linux/arm/v6` из-за отсутствующего armv6 manifest у `gcr.io/distroless/base:debug`. Исправлено: Docker images публикуются только для `linux/amd64` и `linux/arm64`; armv6 release binaries сохраняются. Открыто: повторный release workflow/tag run и public GHCR pull smoke.

Статус 2026-05-31: повторный public `main` snapshot release run `26723315603` подтвердил fix Docker platform matrix: jobs `release`, `release_ui` и `release_ui_darwin` прошли, включая GoReleaser Docker image build path. Новый blocker найден дальше в `Windows Installer / Build Test`: `Download wintun` писал в `${{ github.workspace }}\temp\wintun.zip`, но workflow не создавал `${{ env.downloadPath }}` заранее. Исправлено: перед staging/download steps добавлен `Prepare download directory`, создающий `$env:downloadPath`. Открыто: повторить release workflow после push и подтвердить Windows installer jobs + public GHCR manifests.

Статус 2026-05-31: public `main` snapshot release run `26723908132` после Windows temp-dir fix прошёл полностью зелёным. Подтверждено: `release` OK (`21m48s`), `release_ui` OK, `release_ui_darwin` OK, `Windows Installer / Build Test` OK для `amd64` и `arm64`; оба Windows jobs прошли `Prepare download directory`, `Download wintun`, NSIS build, MSI build и artifact upload. GitHub-hosted standard runners сейчас не являются блокером для public repo; GitHub docs фиксируют free use для standard runners в публичных репозиториях, но larger runners и storage/package limits остаются отдельным release-operations риском. Открыто: настоящий tag/release run, public GHCR manifest pull smoke, clean self-host install из published images и финальный release report.

Статус 2026-06-01: настоящий tag/release gate закрыт на RC2. `v0.72.0-rc.1` выявил tag-only blocker: UI GoReleaser jobs без `contents: write` падали с `403 Resource not accessible by integration`; исправлено для `release_ui` и `release_ui_darwin`. Повторный tag `v0.72.0-rc.2` на commit `a88cbb14c` прошёл полностью зелёным в run `26725448125`: `release_ui` OK (`1m48s`), main `release` OK (`16m21s`), `release_ui_darwin` OK (`2m38s`), Windows Installer Build Test OK для `arm64` (`2m16s`) и `amd64` (`2m57s`), signer job skipped by opt-in. GitHub Release `https://github.com/Cr0me1ve/anonbird/releases/tag/v0.72.0-rc.2` создан с Linux/macOS/Windows, UI, WASM, install scripts и checksums; RC2 вручную помечен как prerelease, а `.goreleaser*.yaml` дополнены `release.prerelease: auto`, чтобы будущие `-rc` теги не маскировались под stable. Проверено: `go run github.com/goreleaser/goreleaser/v2@v2.14.3 check --config .goreleaser.yaml`, `.goreleaser_ui.yaml`, `.goreleaser_ui_darwin.yaml` OK; `actionlint .github/workflows/release.yml` OK.

Статус 2026-06-01: public GHCR image gate закрыт. Server/proxy manifest inspect OK для `ghcr.io/cr0me1ve/anonbird-server:0.72.0-rc.2`, `:latest`, `ghcr.io/cr0me1ve/anonbird-reverse-proxy:0.72.0-rc.2`, `:latest` (`amd64`, `arm64` плюс attestation manifests). Dashboard сначала был доступен только как `:main`; создан и запушен dashboard tag `v0.72.0-rc.2`, workflow `Cr0me1ve/anonbird-dashboard` run `26725937763` прошёл зелёным (`build_n_push` `2m23s`) и опубликовал `ghcr.io/cr0me1ve/anonbird-dashboard:0.72.0-rc.2` + `:latest`, manifest inspect OK (`amd64`, `arm64`, `arm` плюс attestation manifests). Заметка: GitHub Actions сообщил deprecation warning по Node.js 20 actions в dashboard workflow; не блокирует RC2, но нужно учесть до GitHub принудительного Node 24 runner switch.

Статус 2026-06-01: clean one-command self-host из опубликованных release artifacts/images закрыт на `213.108.3.228` без локальных image overrides. Preflight команда `curl -fsSL https://github.com/Cr0me1ve/anonbird/releases/download/v0.72.0-rc.2/getting-started.sh | bash -s -- --domain anonbird-rc2.213.108.3.228.sslip.io --email admin@example.com --proxy traefik --preflight-only` вернула `AnonBird Docker image preflight passed`. Full install той же release script командой с `--yes` подтянул `ghcr.io/cr0me1ve/anonbird-server:latest`, `ghcr.io/cr0me1ve/anonbird-dashboard:latest`, `traefik:v3.6`, поднял `anonbird-traefik`, `anonbird-dashboard`, `anonbird-server`; dashboard `/` вернул `200`, OIDC discovery `/oauth2/.well-known/openid-configuration` вернул `200`, unauthenticated `/api/users` вернул expected `401`. Generated `/etc/anonbird/config.yaml` внутри server содержит `stunPorts: []`, `disableAnonymousMetrics: true`, `disableGeoliteUpdate: true`, `disableVersionCheck: true`; compose grep подтвердил, что `3478/udp` не публикуется. `setup-key bootstrap --config /etc/anonbird/config.yaml` создал bootstrap key с redacted output. `docker logs anonbird-server` grep по `pkgs.netbird`, `github.com/netbirdio`, `GeoLite`, `file will be downloaded`, `NetBird latest`, `fetching version info`, `outdated` пустой. После evidence smoke тестовый compose stack и volumes удалены через `docker compose down --volumes`, generated `/root/docker-compose.yml`, `/root/config.yaml`, `/root/dashboard.env` удалены; unrelated `amnezia-awg2` container оставлен.

Статус 2026-06-01: после RC2 дополнительно разобраны старые public-main CI failures на commit `a88cbb14c`. Причины были open-source workflow blockers, не runtime regression: Linux management jobs обязательно логинились в Docker Hub при пустых `DOCKER_USER`/`DOCKER_TOKEN`; `sync-tag` пытался dispatch upstream/mobile workflows без `NC_GITHUB_TOKEN`; infrastructure docker-compose test собирал старые `netbird-*` binaries, хотя Dockerfile уже ждёт `linux/amd64/anonbird-*`; legacy `getting-started-with-zitadel.sh` job тянул split images `anonbird-management/signal/relay`, которые RC2 не публикует, потому что production quickstart перешёл на combined embedded-Dex server. Исправлено: Docker Hub login steps выполняются только при наличии creds, upstream/mobile sync включается только через explicit repo vars `ANONBIRD_UPSTREAM_SYNC_ENABLED`/`ANONBIRD_MOBILE_SYNC_ENABLED`, infrastructure test builds кладут `anonbird-mgmt/signal/relay` в Dockerfile-compatible `linux/amd64` paths и tag local images as `ghcr.io/cr0me1ve/anonbird-*`, legacy Zitadel CI job сделан opt-in через `ANONBIRD_LEGACY_ZITADEL_CI`. Проверено локально: `actionlint -shellcheck ''` для изменённых workflows OK, `git diff --check` OK; CGO-free signal/relay linux/amd64 output paths build OK locally, management CGO linux build оставлен на Ubuntu CI (local macOS без Linux CGO cross-compiler ожидаемо не собирает runtime/cgo).

Статус 2026-06-01: следующий public-main CI pass на commit `beb138191` выявил оставшиеся public/RC-era blockers, также без признаков runtime regression. `sync-main` пытался dispatch upstream workflow без token; `install-script-test` использовал `/releases/latest`, который игнорирует prerelease `v0.72.0-rc.2`, пока stable release ещё нет; Windows unit workflow скачивал Wintun в несуществующую temp directory и затем маскировал root cause отсутствующим `test-out.txt`; Wasm build вырос до `59331829` bytes и превысил старый 56 MiB лимит; infrastructure compose smoke падал на GeoLite DB, потому что AnonBird production privacy default отключает geolocation download URL. Исправлено локально перед повторным push: `sync-main` opt-in через `ANONBIRD_UPSTREAM_SYNC_ENABLED`; installer принимает semver prerelease tags и `latest` fallback на последний release/pre-release, macOS UI binary install использует опубликованные `anonbird-ui_..._darwin_*.tar.gz` artifacts вместо несуществующего `_signed.zip`, CI pin использует `ANONBIRD_RELEASE=v0.72.0-rc.2` и проверяет `anonbird`; Windows workflow создаёт temp dir, чинит swapped Go cache env и не скрывает primary failure; Wasm budget повышен до 60 MiB; legacy infrastructure compose templates выставляют `NB_DISABLE_GEOLOCATION=true`, а test проверяет disabled geolocation вместо скачивания MaxMind fixtures. Проверено локально: `actionlint -shellcheck ''` для изменённых workflows OK, `bash -n release_files/install.sh infrastructure_files/configure.sh` OK, installer `get_release v0.72.0-rc.2` и `latest` fallback возвращают `v0.72.0-rc.2`, `configure.sh` в temp tree генерирует `NB_DISABLE_GEOLOCATION=true`, `git diff --check` OK. Открыто: запушить hardening commit, дождаться повторного public-main CI, затем продолжить двухклиентную migration/Marton Tor-release suite.

Статус 2026-06-01: public-main CI run `26726469450/455/460/463/453/473/457/458/476` на commit `4d4d3082` подтвердил большую часть hardening: `sync main` skipped, `Test installation` success, `Wasm` success, `Test Infrastructure files` success, `Mobile` success, `Darwin` success, `FreeBSD` success, `Linux` success. Оставшиеся failures: Windows unit tests упали только в `client/cmd` migration tests из-за Windows interpretation of logical Linux paths (`/etc/...`, `/var/backups/...`) under temp root; release workflow main `release`/UI jobs passed, but `Windows Installer / Build Test (amd64)` hit transient timeout downloading external Mesa3D artifact while `arm64` passed. Исправлено локально: `migrationOptions.mapPath` теперь treats slash-prefixed migration paths as logical absolute paths even on Windows and returns slash-style logical backup paths; Windows Wintun/Mesa3D/NSIS external downloads now use PowerShell retry options. Проверено локально: targeted `go test ./client/cmd` migration tests OK, `git diff --check` OK. Открыто: actionlint after workflow retry change, commit/push, repeat CI until Windows + Release are green.

Статус 2026-06-01: public-main CI run на commit `64149ea3d` подтвердил download retry и release path: `Release` success, including main `release`, `release_ui`, `release_ui_darwin`, `Windows Installer / Build Test (amd64)` and `Windows Installer / Build Test (arm64)`; также success/skipped: `sync main`, `Test installation`, `Wasm`, `Test Infrastructure files`, `Mobile`, `Darwin`, `FreeBSD`, `Linux`. Единственный remaining failure был `Windows` unit: `TestApplyClientMigrationWithRejoinHardensConfig` ожидал Unix permission `0600`, но Windows runner reports `0666` for that file mode. Исправлено локально: permission assertion оставлен только для non-Windows; targeted `go test ./client/cmd` OK, `git diff --check` OK. Открыто: запушить финальный Windows-test portability fix и дождаться полностью зелёного public-main run.

Статус 2026-06-01: public-main CI hardening gate закрыт зелёным на commit `7f61dd432` (`Fix Windows migration permission test`). Run set `26727609309/311/315/306/320/318/327/319/326/328` завершился: `Windows` success, `Release` success, `Linux` success, `FreeBSD` success, `Darwin` success, `Mobile` success, `Test Infrastructure files` success, `Wasm` success, `Test installation` success, `sync main` skipped by opt-in. Это подтверждает: Windows migration unit portability fix, release Windows installer external-download retry, prerelease installer pin/fallback, Wasm budget, geolocation-disabled compose smoke и upstream sync opt-in. Следующие release-readiness gates остаются: двухклиентная NetBird baseline migration, Marton Tor relay-only repeat from release artifacts, final release report/verdict.

Статус 2026-06-01: следующий public-main pass после migration/Marton notes выявил flaky Windows CI dependency, не runtime regression: Chocolatey вернул `503` на `sysinternals`, из-за чего `PsExec64` отсутствовал и tests не стартовали. Исправлено в `.github/workflows/golang-test-windows.yml`: Sysinternals теперь имеет direct PSTools.zip fallback с `PsExec64.exe` path export, а PsExec invocations используют `-accepteula`. Проверено локально: `actionlint -shellcheck '' .github/workflows/golang-test-windows.yml`, `git diff --check`. Открыто: дождаться повторного Windows/main CI pass.

Статус 2026-06-01: public-main CI на commit `cf6d8e906` зелёный полностью: `Release` success (`23m57s`), `Linux` success, `Windows` success (`8m48s`, PsExec fallback path confirmed), `Darwin` success, `FreeBSD` success, `Mobile` success, `Wasm` success, `Test installation` success, `Test Infrastructure files` success. Tag `v0.72.0-rc.4` main release workflow прошёл success (`25m9s`), включая `release`, `release_ui`, `release_ui_darwin`, Windows Installer Build Test `arm64` и `amd64`; dashboard tag `v0.72.0-rc.4` также опубликован зелёным. RC4 manifest inspect OK для `ghcr.io/cr0me1ve/anonbird-server:0.72.0-rc.4`, `anonbird-reverse-proxy:0.72.0-rc.4`, `anonbird-dashboard:0.72.0-rc.4`. Follow-up install smoke выявил последний upgrade blocker: Linux installer ставит release binaries, но при apt/dnf/yum detection записывает `package_manager=apt|dnf|yum`, а `--update` принимал только `package_manager=bin` и поэтому отказывался обновлять RC2/dev clients через release script. Исправлено локально: `--update` теперь принимает все release-binary Linux install modes (`bin|apt|dnf|yum`). Проверено: `bash -n release_files/install.sh`, `shellcheck -S error release_files/install.sh`, fake-root update smoke с `package_manager=apt` и `ANONBIRD_RELEASE=v0.72.0-rc.4`. Открыто: запушить fix, получить новый green CI/tag RC, затем повторить update smoke на testbed из опубликованного artifact.

Статус 2026-06-01: public-main CI на commit `a68fe3697` зелёный полностью после release-binary update fix: `Release` success (`24m54s`), `Linux` success, `Windows` success, `Darwin` success, `FreeBSD` success, `Mobile` success, `Wasm` success, `Test installation` success, `Test Infrastructure files` success. Tag `v0.72.0-rc.5` был опубликован для main и dashboard; dashboard image workflow прошёл success (`1m55s`). Main tag release `v0.72.0-rc.5` не считается зелёным: core `release`, `release_ui`, `release_ui_darwin` и Windows installer `arm64` прошли, но Windows installer `amd64` упал на external dependency timeout при скачивании Mesa3D с `downloads.fdossena.com`. Это не runtime regression AnonBird, но release gate остаётся open, потому что RC должен проходить полностью. Исправлено локально: release workflow переведён на общий PowerShell download helper с checksum validation, ручными retry для connection failures и fallback URL `geth.php?r=mesa64-latest` для Mesa3D. Проверено: `actionlint -shellcheck '' .github/workflows/release.yml`, `git diff --check`; локально и на Windows PC `gay--@192.168.0.100` подтверждено, что fallback Mesa3D URL скачивается и даёт SHA256 `71c7cb64ec229a1d6b8d62fa08e1889ed2bd17c0eeede8689daf0f25cb31d6b9`. Открыто: запушить workflow hardening, дождаться зелёного main CI, выпустить следующий RC tag и повторить update/install smoke на testbed из опубликованного artifact.

Статус 2026-06-01: release workflow hardening запушен commit `cf104923f`; public-main CI на нём зелёный полностью (`Release` success `25m45s`, Windows success, Linux/FreeBSD/Darwin/Mobile/Wasm/install/infrastructure success). Tag `v0.72.0-rc.6` main release workflow прошёл success (`25m57s`), включая Windows Installer Build Test `amd64` и `arm64`; dashboard tag `v0.72.0-rc.6` прошёл success (`2m25s`). GitHub Release `v0.72.0-rc.6` создан как prerelease, assets включают `install.sh`, checksums, Linux/macOS/Windows/UI/WASM artifacts; GHCR manifest inspect OK для `anonbird-server:0.72.0-rc.6`, `anonbird-reverse-proxy:0.72.0-rc.6`, `anonbird-dashboard:0.72.0-rc.6`. Live update smoke из опубликованного `rc.6/install.sh` на `185.246.220.249` выявил ещё один production blocker: migrated/manual tar installs могли не иметь `/etc/anonbird/install.conf`, dev/local binaries сравнивались как newer than semver RC, а shadowing `/usr/local/bin/anonbird` мог перехватить `service install` после обновления `/usr/bin/anonbird` и стартовать небезопасный default service unit. На `45.138.103.224` дополнительно подтвердился migration hardening debt: `DisableAutoConnect=true` после `--rejoin` оставляет клиент offline после restart, пока вручную не выполнить `anonbird up`. Исправлено локально: `install.sh --update` теперь treats unowned canonical binary without install.conf as release-binary install, updates non-semver/dev builds to requested semver RC, writes `install.conf` during update, uses absolute canonical binary for service stop/uninstall/install/start, moves shadowing legacy Linux binary to timestamped backup, and restores active connection after update; migration `--rejoin` now flips `DisableAutoConnect=false` after successful anonymous join. Проверено: `bash -n release_files/install.sh`, `shellcheck -S error release_files/install.sh`, fake-root Linux update without install.conf, fake-root dev->semver update, fake-root shadowing `/usr/local` binary removal + reconnect, `go test ./client/cmd -run 'Test.*Migration|TestInitCommands|TestEnableAutoConnectAfterRejoin' -count=1`, `git diff --check`. Live testbed recovered safely: `185` and `45` both run `0.72.0-rc.6`, `command -v anonbird` resolves to `/usr/bin/anonbird`, units point to `/usr/bin/anonbird`, `anonymous-check` OK, management/signal/relay over Tor, ICE endpoints empty, and overlay ping after reconnect works both directions (`185 -> 45` `6/6` avg `820.802 ms`, `45 -> 185` `6/6` avg `797.819 ms`). Открыто: запушить installer/migration fixes, дождаться green CI, выпустить следующий RC artifact and repeat update smoke from published `install.sh` without local patched script.

Статус 2026-06-01: installer/migration fixes запушены commit `1ea13aaa`; public-main CI на нём зелёный полностью (`Release` success `25m2s`, Windows success `9m38s`, Linux/FreeBSD/Darwin/Mobile/Wasm/install/infrastructure success). Tag `v0.72.0-rc.7` main release workflow прошёл success (`25m47s`), dashboard tag `v0.72.0-rc.7` прошёл success (`2m12s`). GitHub Release `v0.72.0-rc.7` создан как prerelease; `install.sh` опубликован, GHCR manifest inspect OK для `ghcr.io/cr0me1ve/anonbird-server:0.72.0-rc.7` (`linux/amd64`, `linux/arm64`), `anonbird-reverse-proxy:0.72.0-rc.7` (`linux/amd64`, `linux/arm64`) и `anonbird-dashboard:0.72.0-rc.7` (`linux/amd64`, `linux/arm64`, `linux/arm/v7`). Published update smoke из `v0.72.0-rc.7/install.sh --update` пройден на `185.246.220.249` и `45.138.103.224`: оба клиента обновились до `0.72.0-rc.7`, `command -v anonbird` -> `/usr/bin/anonbird`, unit points to `/usr/bin/anonbird`, `/etc/anonbird/install.conf` содержит `package_manager=apt`, legacy `netbird` inactive, `DisableAutoConnect=false` после migration rejoin, reconnect path вернул `Already connected`, `anonymous-check` OK. После update оба peer видят друг друга как relay-only через `.onion`, ICE endpoints empty; ping `185 -> 45` `3/3` avg `837.585 ms`, `45 -> 185` `3/3` avg `971.378 ms`. На `185` вручную очищен старый local profile `AdminURL` из ранней pre-fix migration state; fresh migration code уже hardens `AdminURL` to local anonymous control endpoint.

Статус 2026-06-01: stable release path начат. Tag `v0.72.0` опубликован как non-prerelease, `releases/latest/download/getting-started.sh` и `install.sh` начали корректно вести на stable assets; clean one-command self-host из `releases/latest` на `213.108.3.228` прошёл без локальных overrides (`dashboard /` `200`, OIDC discovery `200`, unauth API `401`, `setup-key bootstrap` OK, `stunPorts: []`, `disableAnonymousMetrics: true`, `disableGeoliteUpdate: true`, `disableVersionCheck: true`, no `3478/udp`). Live update smoke с `rc.7` на stable выявил production blocker в installer SemVer compare: `0.72.0-rc.7` ошибочно считался newer than `0.72.0`, поэтому клиенты `185.246.220.249` и `45.138.103.224` не обновлялись из `releases/latest`. Исправлено в `v0.72.1`: stable без prerelease теперь считается newer than same core prerelease; local tests `bash -n`, `shellcheck -S error` и targeted extracted SemVer compare закрыты.

Статус 2026-06-01: tag `v0.72.1` опубликован как stable и содержит Linux/macOS/Windows/UI/WASM/install assets; public-main CI на commit `70c2ea057` зелёный полностью (`Release`, `Linux`, `Windows`, `Darwin`, `FreeBSD`, `Mobile`, `Wasm`, `Test installation`, `Test Infrastructure files`). Dashboard tag `v0.72.1` опубликован зелёным. Main tag workflow `v0.72.1` не считается полностью зелёным: core `release`, `release_ui`, `release_ui_darwin`, Windows installer `arm64` прошли, но Windows installer `amd64` упал на недоступности внешнего Mesa3D host `downloads.fdossena.com` даже после fallback. Это не runtime regression, но stable release gate остаётся open до зелёного tag rerun. Исправлено локально: Windows release workflow переключён на pinned `pal1000/mesa-dist-win` asset `mesa3d-26.1.1-release-msvc.7z` с SHA256 `d5e90e9ae4d620313b61fbbf8e9a55761454e38b6501c39be6d93449c88780e1`; installer staging теперь копирует `opengl32.dll` и required `libgallium_wgl.dll`, а MSI/NSIS package manifests включают оба файла. Проверено локально и на Windows PC `gay--@192.168.0.100`: asset download checksum совпал, archive layout содержит `x64/opengl32.dll` и `x64/libgallium_wgl.dll`, extracted file sizes `139264` и `61549568`; локальный PE import table показывает, что `opengl32.dll` зависит от `libgallium_wgl.dll`. Открыто: push, green main CI, stable patch tag `v0.72.2`, published update smoke из latest на `185`/`45`, release report/verdict.

Статус 2026-06-01: stable `v0.72.2` release gate закрыт. Workflow hardening commit `d06b590cc` запушен в branch и `main`; public-main CI на нём зелёный полностью (`Release` `25m42s`, `Linux` `17m7s`, `Windows` `12m57s`, `Darwin` `9m42s`, `FreeBSD` `13m36s`, `Mobile` `5m28s`, `Wasm` `1m17s`, `Test installation` `44s`, `Test Infrastructure files` `4m6s`). Tag `v0.72.2` main release workflow прошёл success (`27m32s`), включая `release`, `release_ui`, `release_ui_darwin`, Windows Installer Build Test `arm64` и ключевой Windows Installer Build Test `amd64`; dashboard tag `v0.72.2` прошёл success (`2m48s`). GitHub Release `v0.72.2` опубликован как non-prerelease; `releases/latest/download/install.sh` и `getting-started.sh` редиректят на `v0.72.2`. GHCR manifest inspect OK для `anonbird-server:0.72.2` (`linux/amd64`, `linux/arm64`), `anonbird-reverse-proxy:0.72.2` (`linux/amd64`, `linux/arm64`) и `anonbird-dashboard:0.72.2` (`linux/amd64`, `linux/arm64`, `linux/arm/v7`), а также соответствующих `latest` tags. Published latest self-host smoke на `213.108.3.228` пройден из `releases/latest`: preflight OK, dashboard/OIDC/API `200/200/401`, `setup-key bootstrap` OK, config содержит `stunPorts: []`, `disableAnonymousMetrics: true`, `disableGeoliteUpdate: true`, `disableVersionCheck: true`, `3478/udp` не публикуется, server leak grep пустой, тестовый stack/volumes удалены. Published latest update smoke на `185.246.220.249` и `45.138.103.224` пройден: оба клиента обновились с `0.72.0-rc.7` до `0.72.2`, `/usr/bin/anonbird`, `anonbird.service` active, legacy `netbird.service` inactive, `anonymous-check` OK, Tor management/signal/relay, STUN/ICE/direct UDP/fallback disabled, published endpoints none. Overlay validation после update: `185 -> 45` ping `4/4` avg `961.040 ms`, `45 -> 185` ping `4/4` avg `1040.185 ms`; обе стороны видят tested peer как `Relayed` через `.onion`, ICE endpoints `-/-`. Post-update journal grep на `185` и `45` по upstream/version/STUN/ICE/NAT/public-IP patterns пустой. Итоговый release report/verdict создан: `docs/release-report-v0.72.2.md`.
- [x] Поднять реальный тестовый проект через AnonBird virtual network:
...
Статус 2026-06-01: Marton master-backed flow повторён через Tor relay-only AnonBird overlay после NetBird->AnonBird migration. Master поднят на `45.138.103.224` и слушал только overlay `100.79.20.213:18081`/`:18091`, Postgres test container был опубликован только на `127.0.0.1:15432`; edge поднят на `185.246.220.249` и слушал только overlay `100.79.32.188:18082`/`:19092`. Edge запросил master over overlay/gRPC, subscription request `45 -> 185 overlay` вернул Marton config `outbounds=1`, `first_type=wireguard`, `first_server=100.79.20.213`; measured subscription fetch time `6.36s`. Master health from `185 -> 45 overlay` returned `{"status":"ok","db":"ok"}`; edge health and Prometheus metrics from `45 -> 185 overlay` returned `200`. Public negative tests failed as expected: `http://185.246.220.249:18082/healthz` and `http://45.138.103.224:18081/healthz` returned HTTP `000`/connection refused; `ss -ltnp` showed no `0.0.0.0` or public-IP listeners for Marton ports. Marton logs and AnonBird journals after the test marker were clean for public testbed IPs, old NetBird hosts and STUN/ICE/NAT discovery patterns; `anonbird debug anonymous-check` on both peers remained OK (`tor-relay-only`, Tor management/signal/relay, STUN/ICE/direct UDP/fallback disabled, published endpoints none). Temporary Marton processes, Postgres test container and files containing subscription payload were stopped/removed after evidence capture.

Статус 2026-06-01: минимальный Marton master/edge smoke повторён уже поверх опубликованного RC7 клиента (`/usr/bin/anonbird 0.72.0-rc.7`) на Tor relay-only overlay. Master поднят на `45.138.103.224` и слушал только overlay `100.79.20.213:18081`/`:18091`, Postgres test container был опубликован только на `127.0.0.1:15432`; edge поднят на `185.246.220.249` и слушал только overlay `100.79.32.188:18082`/`:19092`. Edge подключился к master gRPC over overlay (`cache invalidation stream connected`), subscription request `45 -> 185 overlay` вернул реальный Marton config payload `outbounds=1`, `first_type=wireguard`, `first_server=100.79.20.213`; edge request duration по log `4089 ms`. Import route вернул `302 marten://import?...url=http://100.79.32.188:18082/sub/<redacted>`, edge metrics over overlay вернули `200`, master health from `185 -> 45 overlay` вернул `{"status":"ok","db":"ok"}`. Public negative tests failed as expected: `http://185.246.220.249:18082/healthz` and `http://45.138.103.224:18081/healthz` returned HTTP `000`/connection refused; `ss -ltnp` showed no public-IP/`0.0.0.0` listeners for Marton ports. `anonbird debug anonymous-check` on both peers remained OK (`tor-relay-only`, Tor management/signal/relay, STUN/ICE/direct UDP/fallback disabled, published endpoints none). Marton master/edge logs and fresh AnonBird journals were clean for public testbed IPs, old NetBird hosts and STUN/ICE/NAT/candidate patterns. Temporary Marton processes, Postgres container, logs and files containing subscription token were stopped/removed after evidence capture; only the copied edge test binary without secrets remains on `185` for repeatability.
- [x] Выполнить release-candidate install/upgrade flow:
...
Закрыто: итоговый release report/verdict и последняя known-limitations сверка перед stable/open-source announcement выполнены в `docs/release-report-v0.72.2.md`; отдельный русский release report добавлен в `docs/release-report-v0.72.2-ru.md`. Clean install server/dashboard из опубликованных images, RHEL-like client install, published latest update smoke, server-side NetBird baseline migration + rollback, двухклиентный client migration + rollback/reapply и Marton Tor/I2P overlay checks закрыты отдельными E2E/status entries.
- [x] Перед публичным релизом создать release report:
  - commit/tag;
  - artifact checksums;
  - test matrix;
  - known limitations;
