...
- Оценка остатка на 2026-06-01 после open-source hygiene, GitHub/local rename, public repo switch, зелёного RC2 tag/release workflow, public GHCR manifests, clean one-command self-host из опубликованных образов, зелёного public-main CI, двухклиентной NetBird->AnonBird client migration с rollback/reapply и Marton Tor overlay repeat: примерно 98.6% от ТЗ закрыто; главный незакрытый блок — новый полностью зелёный RC/release artifact с последним migration hardening, repeat ключевых smoke из этого artifact и финальный release report/verdict.
