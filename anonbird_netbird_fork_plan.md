...
Статус 2026-05-31: one-command self-host script дополнительно hardened для release artifacts: добавлены image overrides (`ANONBIRD_DASHBOARD_IMAGE`, `ANONBIRD_SERVER_IMAGE`, `ANONBIRD_PROXY_IMAGE`), `--preflight-only`, `--skip-image-preflight` и Docker image preflight перед запуском контейнеров. Если AnonBird release images не опубликованы/приватны/недоступны, installer теперь падает до `docker compose up` с точным списком образов и командами исправления. Проверено локально: `bash -n`, `shellcheck -S error`, `--help`, `--render-only` + `docker compose config`, preflight failure для missing images и skip path. Открыто: опубликовать/подтвердить реальные `ghcr.io/cr0me1ve/anonbird-*` release images и прогнать полный server/dashboard clean install на testbed.
