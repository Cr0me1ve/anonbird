...
func (s *SqlStore) getPeers(ctx context.Context, accountID string) ([]nbpeer.Peer, error) {
	...
	meta_environment, meta_flags, meta_files, meta_capabilities, meta_anonymous_transport, meta_i2p_destination,
	peer_status_last_seen, peer_status_session_started_at,
	...
	peers, err := pgx.CollectRows(rows, func(row pgx.CollectableRow) (nbpeer.Peer, error) {
		...
		var (
			...
			metaAnonymousTransport, metaI2PDestination                                                      sql.NullString
		...
		err := row.Scan(&p.ID, &p.AccountID, &p.Key, &ip, &p.Name, &p.DNSLabel, &p.UserID, &p.SSHKey, &sshEnabled,
			...
			&metaAnonymousTransport, &metaI2PDestination, &peerStatusLastSeen, &peerStatusSessionStartedAt, &peerStatusConnected, &peerStatusLoginExpired,
		...
		if err == nil {
			...
			if metaAnonymousTransport.Valid {
				p.Meta.AnonymousTransport = metaAnonymousTransport.String
			}
			if metaI2PDestination.Valid {
				p.Meta.I2PDestination = metaI2PDestination.String
			}
