...
var sshCmd = &cobra.Command{
	...
	Short: "Connect to an AnonBird peer via SSH",
	Long: `Connect to an AnonBird peer using SSH with support for port forwarding.
...
Examples:
  anonbird ssh peer-hostname
  anonbird ssh root@peer-hostname
  anonbird ssh --login root peer-hostname
  anonbird ssh peer-hostname ls -la
  anonbird ssh peer-hostname whoami
  anonbird ssh -t peer-hostname tmux                  # Force PTY for tmux/screen
  anonbird ssh -t peer-hostname sudo -i               # Force PTY for interactive sudo
  anonbird ssh -L 8080:localhost:80 peer-hostname     # Local port forwarding
  anonbird ssh -R 9090:localhost:3000 peer-hostname   # Remote port forwarding
  anonbird ssh -L "*:8080:localhost:80" peer-hostname # Bind to all interfaces
  anonbird ssh -L 8080:/tmp/socket peer-hostname      # Unix socket forwarding`,
...
func runSSH(ctx context.Context, addr string, cmd *cobra.Command) error {
	...
	if err != nil {
		...
		cmd.Printf("  1. Check peer connectivity: anonbird status -d\n")
...
var sshDetectCmd = &cobra.Command{
	...
	Short:  "Detect if a host is running AnonBird SSH",
	Long:   "Internal command used by SSH Match exec to detect AnonBird SSH servers. Exit codes: 0=JWT, 1=no-JWT, 2=regular SSH",
	Hidden: true,
	Args:   cobra.ExactArgs(2),
	RunE:   sshDetectFn,
}
