...
var statusCmd = &cobra.Command{
	Use:   "status",
	Short: "Display AnonBird client status",
	Long:  "Display the current status of the AnonBird client, including connection status, peer information, and network details.",
	RunE:  statusFunc,
}
...
func init() {
	...
	statusCmd.PersistentFlags().BoolVarP(&ipv4Flag, "ipv4", "4", false, "display only AnonBird IPv4 of this peer, e.g., --ipv4 will output 100.64.0.33")
	statusCmd.PersistentFlags().BoolVarP(&ipv6Flag, "ipv6", "6", false, "display only AnonBird IPv6 of this peer")
	...
	statusCmd.PersistentFlags().StringSliceVarP(&prefixNamesFilter, "filter-by-names", "N", []string{}, "filters the detailed output by a list of one or more peer FQDN or hostnames, e.g., --filter-by-names peer-a,peer-b.example.com")
...
func statusFunc(cmd *cobra.Command, args []string) error {
	...
	if needsAuth && !jsonFlag && !yamlFlag {
		cmd.Printf("Daemon status: %s\n\n"+
			...
			"you can use a setup-key:\n\n anonbird up --management-url <YOUR_MANAGEMENT_URL> --setup-key <YOUR_SETUP_KEY>\n\n"+
			"Create setup keys in your management dashboard.\n\n",
			resp.GetStatus(),
		)
		return nil
	}
