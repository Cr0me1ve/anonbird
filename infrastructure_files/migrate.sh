#!/bin/bash
#
# AnonBird Migration Script: legacy NetBird pre-v0.65.0 -> combined container setup
#
# Migrates from the old 5-container deployment (dashboard, signal, relay, management, coturn)
# to the new 2-container AnonBird setup (Traefik + combined anonbird-server).
#
...
print_banner() {
  echo ""
  echo "$MSG_SEPARATOR"
  echo "  AnonBird Migration Tool v${SCRIPT_VERSION}"
...
detect_volumes() {
  ...
  if [[ -n "$compose_file" ]]; then
    # Look for volume mount on /var/lib/netbird in management or anonbird-server service
    local vol_name
...
# AnonBird Migration Rollback Script
...
echo "Restoring AnonBird configuration from backup..."
...
services:
  ...
  dashboard:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.anonbird-dashboard.rule=Host(\`${DOMAIN}\`)
  ...
  anonbird-server:
    ...
    labels:
      ...
      - traefik.http.routers.anonbird-grpc.rule=Host(\`${DOMAIN}\`) && (PathPrefix(\`/signalexchange.SignalExchange/\`) || PathPrefix(\`/management.ManagementService/\`))
      ...
      - traefik.http.routers.anonbird-backend.rule=Host(\`${DOMAIN}\`) && (PathPrefix(\`/relay\`) || PathPrefix(\`/ws-proxy/\`) || PathPrefix(\`/api\`) || PathPrefix(\`/oauth2\`))
...
wait_for_health() {
  ...
  while [[ $attempt -lt $max_attempts ]]; do
    ...
    if [[ $attempt -eq 30 ]]; then
      ...
      (cd "$INSTALL_DIR" && $DOCKER_COMPOSE_CMD logs --tail=10 anonbird-server 2>/dev/null) || true
...
print_summary() {
  ...
  echo "    - Clients should reconnect automatically; if not: anonbird down && anonbird up"
  echo ""
  echo "  Next steps:"
  echo "    - Access the dashboard: https://$DOMAIN"
  echo "    - Re-authenticate all clients: anonbird down && anonbird up"
